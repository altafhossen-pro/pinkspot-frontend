...
export default function FloatingContact() {
  // Default to standard number, but fetch dynamically
  ...
  useEffect(() => {
    ...
    // Fetch dynamic contact number from backend settings
    const fetchSettings = async () => {
      try {
        const response = await settingsAPI.getSiteSettings();
        if (response?.success && response?.data?.contact?.phone) {
          setPhoneNumber(response.data.contact.phone);
        }
      } catch (error) {
        console.error('Failed to load dynamic contact for floating button', error);
      }
    };
